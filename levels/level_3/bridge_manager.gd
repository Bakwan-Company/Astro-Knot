...
func _on_terminal_activated(is_on: bool) -> void:
	...
	if is_on:
		...
		tween.tween_property(left_bridge, "global_position", left_formed_pos, slide_duration)
		tween.tween_property(right_bridge, "global_position", right_formed_pos, slide_duration)
	else:
		# TERMINAL MATI: MATIIN FISIK SAAT INI JUGA!
		_set_bridge_collision(false)
		
		# Baru jembatannya ditarik ngumpet ke tembok (visual doang yang gerak)
		tween.tween_property(left_bridge, "global_position", left_hidden_pos, slide_duration)
		tween.tween_property(right_bridge, "global_position", right_hidden_pos, slide_duration)
